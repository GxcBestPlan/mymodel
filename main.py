...
parser.add_argument('--hr_test_dataset', type=str, default='DIV2K_valid_HR')
...
def train(epoch):
    epoch_loss = 0
    model.train()
    with tqdm(total=(len(train_set) - len(train_set) % opt.batchSize)) as t:
        t.set_description('epoch: {}/{}'.format(epoch, opt.nEpochs))
...
def checkpoint(epoch):
    model_out_path = opt.save_folder+opt.hr_train_dataset+opt.model_type+opt.prefix+"_epoch_{}.pth".format(epoch)
...
test_set = get_test_set(opt.data_dir, opt.hr_test_dataset, opt.upscale_factor, opt.patch_size, opt.data_augmentation)
